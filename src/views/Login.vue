<template>
    ...
    <div class="max-w-md mx-auto mt-4 bg-white p-6 rounded-lg shadow-md">
      <h2 class="text-xl font-bold mb-4 ">Login account</h2>

      <form @submit.prevent="loginUser">
        <div class="mb-4">
          <label class="block text-gray-700 font-medium ">Email</label>
          <input v-model="email" type="text" class="mt-1 p-2 w-full border rounded-md" placeholder="enter your email">
        </div>
...
<script>
    ...
    methods:{
        async loginUser(){
            this.loading = true 
            try {
                const response = await axios.post('https://codecrafter-back-tcc5.onrender.com/api/users/login', {
                    email: this.email,
